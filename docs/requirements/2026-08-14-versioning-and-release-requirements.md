# Versioning and ZIP-Only Release Requirements
...
For every delivery, increment the patch component exactly once per
...
Project files, production source, UI text, package names, and release metadata
must derive their versions from this source rather than duplicating a literal.

### 3. ZIP-only release and data safety
...
executable, packaged README, application icon, and third-party notices while
preserving all existing files and contents under the payload's `data` and `logs`
directories. The portable ZIP is the sole distributable application package;
`SHA256SUMS.txt` and `verification.txt` are release metadata only.

The ZIP payload must contain exactly the executable, application icon,
`README.txt`, `THIRD-PARTY-NOTICES.md`, and empty `data` and `logs` directories.
The publish directory may retain local runtime files under `data` and `logs`,
but those files must never enter the ZIP.
...
must each pass `--self-test` with exit code `0`. Validate the published payload
while allowing preserved local runtime data, validate the staged and extracted
ZIP payloads with empty runtime directories, and confirm the ZIP contains no
installer, uninstaller, installation script, credential cache, account cache,
token cache, or `rclone` executable. The release directory must contain only
the versioned ZIP before metadata creation. `SHA256SUMS.txt` must contain one
lowercase checksum entry for that ZIP, and `verification.txt` must record the
publish build, both self-tests, both payload validations, ZIP creation,
artifact-set validation, and SHA-256. The final report must include the
executable path and SHA-256, test count, and the before/after preservation
result for `data` and `logs`.

## Historical delivery record: 1.1.1
...
- the published `1.1.1` payload passed the verification gates in effect at
  that time.
